...
if missing:
    raise ValueError(
        f"CSV에 필요한 컬럼이 없습니다: {missing}. 현재 컬럼: {list(train_df.columns)}"
    )
...
def predict_category(question):
    ...
    # 1. 명확한 학과정보 질문 우선 처리
    # 학과명이 포함되어 있어도 교수/연락처/연구실 질문이면 학과정보로 보냄
    if contains_any(q, ["교수", "교수님", "연구실", "이메일", "연락처", "전화", "학과장", "전임교수"]):
        return "학과정보", 1.0, "rule-department-info"

    # 2. 졸업인증 질문 우선 처리
    if contains_any(q, ["졸업인증", "심폐소생술", "사회봉사", "토익", "오픽", "ITQ", "MOS", "정보처리"]):
        return "졸업인증", 1.0, "rule-graduation"

    # "인증" 단독은 너무 넓으므로 졸업/자격/어학 등과 같이 있을 때만 처리
    if "인증" in q and contains_any(q, ["졸업", "자격", "어학", "토익", "오픽", "사회봉사", "심폐소생술"]):
        return "졸업인증", 1.0, "rule-graduation"

    # 3. 대체과목 질문 처리
    if contains_any(q, ["대체", "폐강", "폐지", "없어졌", "없어진", "복학생", "미이수", "대신"]):
        return "대체과목", 1.0, "rule-replacement"

    # 4. 일정 질문 처리
    # 기존 "어디"는 너무 넓어서 제외
    if contains_any(q, ["언제", "날짜", "일정", "기간", "몇월", "며칠", "캘린더"]):
        if not contains_any(
            q,
            [
                "교육과정",
            ...
            return "일정", 1.0, "rule-schedule"

    if contains_any(q, ["장소", "어디서"]):
        if not contains_any(q, ["교수", "연구실", "이메일", "연락처"]):
            return "일정", 1.0, "rule-schedule-place"

    # 5. 줄임말 / 뜻 질문 처리
    # "뭐야"는 너무 넓어서 제외
    if contains_any(q, ["뜻", "줄임말", "무슨 뜻", "정식명칭", "정식 명칭"]):
        return "학과정보", 1.0, "rule-alias-info"
    ...
    # 8. NB가 애매하면 CSV 질문 transformer로 보조
    if prob < 0.35:
        ...
        if best_score < 0.45:
            return "모호함", best_score, "csv-transformer-low-score"

        return train_categories[idx], best_score, "csv-transformer"
...
def answer_department_info(question):
    ...
    for alias, full in ALIAS_MAP.items():
        if compact(alias) in cq and contains_any(q, ["뜻", "줄임말", "무슨 뜻", "정식명칭", "정식 명칭"]):
...
def semantic_chunk_answer(question, category_filter=""):
    ...
    # 기존 0.32는 낮아서 관련 없는 답변이 나올 수 있음
    if score < 0.45:
        return (
            "관련 정보를 정확히 찾지 못했습니다. "
            "학과명, 학년/학기, 과목명, 일정명 등을 조금 더 구체적으로 입력해주세요."
        )
...
def answer_question(question):
    ...
    if category == "모호함":
        answer = (
            "질문이 조금 모호합니다. "
            "학과명, 학년/학기, 과목명, 일정명 등을 포함해서 다시 질문해주세요."
        )
